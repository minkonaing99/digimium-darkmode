...
(() => {
  // ---------- Config ----------
  const API_URL = "api/sales_minimal.php"; // returns: id, product_name, price, profit, purchase_date
  ...
  const todayYMD = () => {
    ...
    console.log("Today's date (YMD):", result);
    return result;
  };
  ...
  function updateKPIs(rows) {
    console.log("Updating KPIs with rows:", rows);
    ...
    for (const r of rows) {
      ...
      console.log("Row:", { date: d, price, profit, product: r.product_name });
  ...
  function buildDailyPies(rows) {
    const today = todayYMD();
    console.log("Building daily pies for today:", today);

    // group today's rows by product
    const agg = new Map(); // product -> {sales, profit, count}
    for (const row of rows) {
      console.log("Processing row:", row);
      if (String(row.purchase_date) !== today) {
        console.log("Skipping row - not today's date");
        continue;
      }
    ...
    console.log("Aggregated data:", Object.fromEntries(agg));
  ...
  async function buildLine30(rows) {
    ...
    for (const row of rows) {
      const k = String(row.purchase_date || "");
