...
(() => {
  ...
  function buildSearchKey(r) {
    ...
    const purchasedTokens = tokenizeDate(r.purchase_date);
    const expiredTokens = tokenizeDate(r.end_date);
  ...
  function updateLocalRow(id, patch) {
    ...
    const touchesSearch =
      ...
      "gmail" in patch ||
      "seller" in patch ||
      "product_name" in patch ||
      "purchase_date" in patch ||
      "end_date" in patch;
    ...
    if (cached) {
      try {
        ...
        const idx = data.findIndex((r) => String(r.id) === idStr);
        if (idx >= 0) {
  ...
  function buildSaleTr(s, displayNum) {
    ...
    if (s.id != null) tr.dataset.id = String(s.id);
    ...
    tdPurchased.textContent = formatDate(s.purchase_date);
  ...
  // total columns = 11, price is the 2nd-to-last column.
  const TOTAL_COLS = 11;
  const PRICE_COL_INDEX = TOTAL_COLS - 2; // 9
  ...
  function buildSubtotalTr(dateKey) {
    ...
    // Duration (idx 2), Manager (idx 7), Note (idx 8) → 3 fillers.
  ...
  function startInlineEdit(td) {
    ...
    const field = td.dataset.field; // customer|gmail|seller|note
    ...
    input.type = field === "gmail" ? "email" : "text";
  ...
  async function saveInline(td) {
    ...
    // tiny validation
    if (field === "gmail" && next && !/^\S+@\S+\.\S+$/.test(next)) {
  ...
  function appendNextChunkCards() {
    ...
    for (let i = start; i < end; i++) {
      ...
      const email = esc(r.gmail ?? "-");
      const manager = esc(r.seller ?? "-");
      const purchased = formatDate(r.purchase_date);
      const expired = formatDate(r.end_date);
      ...
      article.innerHTML = `
        ...
        <div class="subs-row subs-name">
          <span class="subs-label">Seller:</span>
          <span>${manager}</span>
        </div>
  ...
  tbody?.addEventListener("click", async (e) => {
    ...
    if (!id) return alert("Missing id for this row.");
...
(async () => {
  ...
  if (elPurchase && !elPurchase.value) {
    ...
    console.log("Set purchase date to:", elPurchase.value);
  }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    console.log(
      "Product selected:",
      elProduct?.selectedOptions?.[0]?.textContent
    );
    console.log("Purchase date:", elPurchase?.value);
    console.log("Customer:", elCustomer?.value);
    if (!validate()) {
    ...
    // payload:
    const payload = {
      product_name: saleName,
      duration: Number.isFinite(duration) ? duration : null,
      customer: (elCustomer?.value || "").trim(),
      gmail: (elEmail?.value || "").trim() || null,
      purchase_date: elPurchase?.value,
      end_date: elEndDate?.value || null,
      seller: (elSeller?.value || "").trim() || null,
