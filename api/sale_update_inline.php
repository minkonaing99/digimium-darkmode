...
try {
    ...
    $allowed = ['customer', 'gmail', 'seller', 'note'];
    ...
    // Support both shapes: {id, field, value} OR {id, customer/gmail/seller/note}
    ...
    if (!$field || !in_array($field, $allowed, true)) {
        json_fail('Missing or unsupported "field". Allowed: customer, gmail, seller, note.', 422);
    }
    ...
    switch ($field) {
        ...
        case 'gmail':
            ...
            if ($value !== null) {
                ...
                if (mb_strlen($value) > 255) json_fail('"gmail" too long (max 255).', 422);
            }
            break;

        case 'seller':
            if ($value === '') $value = null; // allow clearing
            if ($value !== null && mb_strlen($value) > 255) json_fail('"seller" too long (max 255).', 422);
            break;
    ...
    $sql = "UPDATE product_sold SET {$field} = :val WHERE id = :id LIMIT 1";
    ...
    if ($stmt->rowCount() === 0) {
        ...
        $check = $pdo->prepare("SELECT COUNT(*) FROM product_sold WHERE id = :id");
        $check->execute([':id' => $id]);
