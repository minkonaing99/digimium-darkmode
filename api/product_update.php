<?php
// api/product_update.php
header('Content-Type: application/json; charset=utf-8');
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: POST'); // POST only

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    http_response_code(405);
    header('Allow: POST');
    echo json_encode(['success' => false, 'error' => 'Method not allowed. Use POST.']);
    exit;
}

require_once __DIR__ . '/dbinfo.php';

try {
    if (!isset($pdo) || !($pdo instanceof PDO)) {
        throw new RuntimeException('PDO connection not initialized.');
    }
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);

    $raw  = file_get_contents('php://input');
    $data = json_decode($raw, true);
    if (!is_array($data)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Invalid JSON payload.']);
        exit;
    }

    // Required: id
    $id = isset($data['id']) ? (int)$data['id'] : 0;
    if ($id <= 0) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Missing or invalid id.']);
        exit;
    }

    // Optional updatable fields
    $MAX_VARCHAR = 255;
    $MAX_URL_LEN = 2083;

    $trimOrNull = function ($v) {
        if ($v === null) return null;
        $s = trim((string)$v);
        return $s === '' ? null : $s;
    };
    $toInt      = function ($v) {
        if ($v === '' || $v === null || $v === false) return null;
        if (!is_numeric($v)) return null;
        return (int)$v;
    };
    $toDecStr   = function ($v) {
        if ($v === '' || $v === null || $v === false) return null;
        if (!is_numeric($v)) return null;
        return number_format((float)$v, 2, '.', '');
    };


    $fields = [];
    $params = [':id' => $id];
    $errors = [];

    // product_name
    if (array_key_exists('product_name', $data)) {
        $product_name = $trimOrNull($data['product_name']);
        if (!$product_name) $errors['product_name'] = 'Product name cannot be empty.';
        elseif (mb_strlen($product_name) > $MAX_VARCHAR) $errors['product_name'] = "Max {$MAX_VARCHAR} characters.";
        else {
            $fields[] = 'product_name = :product_name';
            $params[':product_name'] = $product_name;
        }
    }

    // duration
    if (array_key_exists('duration', $data)) {
        $duration = $toInt($data['duration']);
        if (!is_int($duration) || $duration < 1) $errors['duration'] = 'Duration must be an integer ≥ 1.';
        else {
            $fields[] = 'duration = :duration';
            $params[':duration'] = $duration;
        }
    }



    // supplier
    if (array_key_exists('supplier', $data)) {
        $supplier = $trimOrNull($data['supplier']);
        if ($supplier !== null && mb_strlen($supplier) > $MAX_VARCHAR) $errors['supplier'] = "Max {$MAX_VARCHAR} characters.";
        else {
            $fields[] = 'supplier = :supplier';
            $params[':supplier'] = $supplier;
        }
    }

    // wc_price
    if (array_key_exists('wc_price', $data)) {
        $wc_price = $toDecStr($data['wc_price']);
        if ($wc_price === null || (float)$wc_price < 0) $errors['wc_price'] = 'WC price must be a number ≥ 0.';
        else {
            $fields[] = 'wc_price = :wc_price';
            $params[':wc_price'] = $wc_price;
        }
    }

    // retail_price
    if (array_key_exists('retail_price', $data)) {
        $retail_price = $toDecStr($data['retail_price']);
        if ($retail_price === null) $errors['retail_price'] = 'Retail price is required.';
        else {
            $params[':retail_price'] = $retail_price;
            $fields[] = 'retail_price = :retail_price';
        }
    }

    // notes
    if (array_key_exists('notes', $data)) {
        $notes = $trimOrNull($data['notes']);
        $fields[] = 'notes = :notes';
        $params[':notes'] = $notes;
    }

    // link (normalize)
    if (array_key_exists('link', $data)) {
        $link = $trimOrNull($data['link']);
        if ($link !== null) {
            if (!preg_match('~^https?://~i', $link)) $link = 'https://' . $link;
            $link = preg_replace('/\s+/', '%20', $link);
        }
        $fields[] = 'link = :link';
        $params[':link'] = $link;
    }

    if ($errors) {
        http_response_code(422);
        echo json_encode(['success' => false, 'errors' => $errors]);
        exit;
    }

    if (!$fields) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'No updatable fields provided.']);
        exit;
    }

    // Enforce retail_price > wc_price when changed
    if (in_array('retail_price = :retail_price', $fields, true) || in_array('wc_price = :wc_price', $fields, true)) {
        $curr = $pdo->prepare('SELECT wc_price, retail_price FROM product_list WHERE product_id = :id');
        $curr->execute([':id' => $id]);
        $row = $curr->fetch();
        if (!$row) {
            http_response_code(404);
            echo json_encode(['success' => false, 'error' => 'Record not found.']);
            exit;
        }
        $finalWcPrice = isset($params[':wc_price']) ? (float)$params[':wc_price'] : (float)$row['wc_price'];
        $finalRetailPrice = isset($params[':retail_price']) ? (float)$params[':retail_price'] : (float)$row['retail_price'];
        if ($finalRetailPrice <= $finalWcPrice) {
            http_response_code(422);
            echo json_encode(['success' => false, 'errors' => ['retail_price' => 'Retail must be greater than WC price.']]);
            exit;
        }
    }

    $sql = 'UPDATE product_list SET ' . implode(', ', $fields) . ' WHERE product_id = :id';
    $stmt = $pdo->prepare($sql);
    $stmt->execute($params);

    if ($stmt->rowCount() === 0) {
        $exists = $pdo->prepare('SELECT 1 FROM product_list WHERE product_id = :id');
        $exists->execute([':id' => $id]);
        if (!$exists->fetchColumn()) {
            http_response_code(404);
            echo json_encode(['success' => false, 'error' => 'Record not found.']);
            exit;
        }
        // no-op update
    }

    // Return updated row
    $out = $pdo->prepare('SELECT product_id, product_name, duration, supplier, wc_price, retail_price, notes, link FROM product_list WHERE product_id = :id');
    $out->execute([':id' => $id]);
    $row = $out->fetch();

    echo json_encode(['success' => true, 'row' => $row]);
} catch (Throwable $e) {
    http_response_code(500);
    echo json_encode(['success' => false, 'error' => $e->getMessage()]);
}
